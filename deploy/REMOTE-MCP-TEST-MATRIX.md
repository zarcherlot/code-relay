...
| Positive | Publish runbook through Contents API | `runbooks/<id>/runbook.md` is committed and checkpoint workflow is dispatched | `TestGitHubRemotePublishAndDispatch` |
| Positive | Fetch/analyze receipt from GitHub Contents API | Receipt is validated against the runbook and analyzed | `GitHubRemoteBackend.fetchReceipt` path |
...
| Boundary | Invalid repository, branch, workflow, runbook ID, or runbook size | Validation error; no GitHub mutation | normalization and contract tests |
...
| Staging | Negative unauthorized repository/branch and malformed runbook | No commit/dispatch; MCP error is redacted | Run with staging App and test repository |
