...
def _git(root: Path, *args: str) -> subprocess.CompletedProcess[str]:
    try:
        return subprocess.run(["git", *args], cwd=root, text=True, capture_output=True, check=False, timeout=GIT_TIMEOUT_SECONDS)
    except subprocess.TimeoutExpired as exc:
        raise ProtocolError(f"git 操作超时（{GIT_TIMEOUT_SECONDS}s）: {' '.join(args)}") from exc


def _repo_root(value: str | None) -> Path:
    root = Path(value or os.environ.get("CODE_RELAY_ROOT", ".")).resolve()
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    report = doctor(_repo_root(args.root))
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    else:
        print(f"Code Relay doctor: {report['status']}")
...
def cmd_project_bind(args: argparse.Namespace) -> int:
    ...
    if args.role == "orchestrator" and not args.no_invite:
        invite = create_invite(root, args.expires, not args.reusable)
...
def cmd_invite(args: argparse.Namespace) -> int:
    ...
    invite = create_invite(_repo_root(args.root), args.expires, not args.reusable)
    print(invite["url"])
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor = sub.add_parser("doctor", help="检查本地 Git、绑定、权限和运行时")
    ...
    bind.add_argument("--reusable", action="store_true", help="允许同一邀请链接重复加入（默认一次性）")
    ...
    invite.add_argument("--reusable", action="store_true", help="允许同一邀请链接重复加入（默认一次性）")
