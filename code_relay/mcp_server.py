...
TOOLS = [
    {"name": "bind_project", "description": "Bind the current project and branch to Code Relay and create the verifier invite for an orchestrator.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string"}, "role": {"enum": ["orchestrator", "verifier"]}, "ref": {"type": "string"}, "expires": {"type": "integer", "minimum": 5, "maximum": 1440}, "one_time": {"type": "boolean", "default": True}}, "required": ["role"]}},
    {"name": "create_verifier_invite", "description": "Create a short-lived verifier join link for the bound project; invites are one-time by default.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string"}, "expires": {"type": "integer", "minimum": 5, "maximum": 1440}, "one_time": {"type": "boolean", "default": True}}, "required": []}},
    {"name": "join_verifier", "description": "Join a branch-scoped Code Relay verifier subscription using a join link and start its Go watcher by default. Use runtime=python only for explicit development debugging.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string"}, "url": {"type": "string"}, "destination": {"type": "string"}, "poll_interval": {"type": "number", "minimum": 1}, "runtime": {"enum": ["python", "go"], "default": "go"}}, "required": ["url"]}},
    ...
    {"name": "doctor", "description": "Run non-mutating local Code Relay health checks.", "inputSchema": {"type": "object", "properties": {"root": {"type": "string"}}, "required": []}},
]
...
def handle(request: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        return {"jsonrpc": "2.0", "id": request_id, "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "code-relay", "version": "1.0.0"}}}
    if method == "ping":
        ...
        try:
            if name == "bind_project":
                ...
                if args["role"] == "orchestrator":
                    value["invite"] = create_invite(root, int(args.get("expires", 30)), bool(args.get("one_time", True)))
            elif name == "create_verifier_invite":
                value = create_invite(root, int(args.get("expires", 30)), bool(args.get("one_time", True)))
